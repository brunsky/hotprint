...
a?a:"https://api.instagram.com/v1/users/"+user_id+"/media/recent",data:{access_token:access_token,count:100}}).success(_displayUserRecent)};_displayUserRecent=function(a){for(var b=0;b<a.data.length;b++)h+="<li><a><img src='loader.gif' delay_src=\""+a.data[b].images.standard_resolution.url+"\" id='recent_instaimg"+b+"' class='draggable draggable-h draggable-w gallery-pool' /></a></li>";"undefined"!=typeof a.pagination.next_url?_getUserRecent(a.pagination.next_url):gallery_bar_setting(h)};var e;_fblogin=
function(a){$(".recent").find(".draggable").each(function(){$(this).remove();$(this)[0]=null});e="";var b="http://www.facebook.com/dialog/oauth/?client_id=290426614419964&redirect_uri="+HOST_URL+"fbredirect.html&response_type=token&scope=user_photos";if(!0==a.data.isPopup)return window.open(b,"hotprintCloud","height=600,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=yes"),!1;window.location=b};_fbCrossLogin=function(a){$("#facebook-source").html("");
access_token=a;source_type="facebook";null==$.cookie("source")&&$.cookie("source",source_type);$("body").addClass("logged-in");f();FB.api("/me?access_token="+access_token+"&fields=albums,id,name",function(a){!a||a.error?($.removeCookie("source"),$.removeCookie("access_token"),$("body").removeClass("logged-in"),$("body").addClass("not-logged-in")):(user_id=a.id,"facebook"==$.cookie("source")&&($.cookie("user_id",user_id),$.cookie("user_type","facebook"),$(".open").html("").fadeIn(300).append(a.name)),
...
h.source+"\" id='recent_fbimg"+c+i+"' class='draggable draggable-h gallery-pool'/></a></li>"):e+("<li><a><img src='loader.gif' delay_src=\""+h.source+"\" id='recent_fbimg"+c+i+"' class='draggable draggable-w gallery-pool'/></a></li>")}"undefined"!=typeof a.paging?"undefined"!=typeof a.paging.next?_facebookPhotoAlbum(a.paging.next):gallery_bar_setting(e):gallery_bar_setting(e)})}})(jQuery);
...
Math.round(0.7*$(this).width()))},stop:function(){enable_scroll();isCornerFading=!0;$(".layout_corner").animate({opacity:CORNER_OPT},{complete:function(){$(".layout_corner > .draggable").parent().css("opacity","0");isCornerFading=!1}})}})}
