...
var TEMP_DIR="proxy/tmp/",PHONE_NAME="iphone5",LAYOUT_NAME="layout_3",PHONE_COLOR="white",layout_ox=410,layout_oy=Math.floor(0.1*$(window).height()),CORNER_OPT=0.1,isCornerFading=!1,PAGE="Design",TITLE_NAME="",maskImg,isMenuEntry=!1,MY_LANG="en",DOLLAR_SIGN="$",_init,_getUserData,_displayUserData,_instaCrossLogin,_instalogin,_instalogout,_getUserRecent,_facebookPhotoAlbum,_fblogin,_fbCrossLogin,_fblogout,_logout,_loadLayout,access_token,user_id,source_type;
(function(){function a(a){!1==isMenuEntry&&(isMenuEntry=!0,menuLoadLayout(a.data.param))}function d(a){!1==isMenuEntry&&(isMenuEntry=!0,menuLoadPhone(a.data.param))}function c(a){!1==isMenuEntry&&(isMenuEntry=!0,menuLoadColor(a.data.param))}function f(){$("#save-design").unbind("click");$("#random-design").unbind("click");$("#clear-design").unbind("click");$("#start-design").unbind("click");$("#my-gallery").unbind("click");$("#my-order-history").unbind("click");$("#logout").unbind("click");$("#go-to-my-gallery").unbind("click");
$("#go-to-my-order-history").unbind("click");$("#save-design").bind("click",saveImg);$("#random-design").bind("click",randomDesign);$("#clear-design").bind("click",clearDesign);$("#start-design").bind("click",startDesign);$("#my-gallery").bind("click",openGallery);$("#logout").bind("click",_logout);$("#go-to-my-gallery").bind("click",openGallery);$("#go-to-my-order-history").bind("click",openHistory)}$.extend({social_connect:function(){_init()}});_init=function(){$("html").removeClass("no-js");$("body").addClass("not-logged-in");
$("#login").click({isPopup:!1},_instalogin);$("#login2").click({isPopup:!1},_fblogin);$("#facebook-source-login").click({isPopup:!0},_fblogin);$("#instagram-source-login").click({isPopup:!0},_instalogin);$(".layout_item").each(function(){$(this).click({param:$(this).attr("id")},a)});$(".phone_item").each(function(){$(this).click({param:$(this).attr("id")},d)});$(".color_item").each(function(){$(this).click({param:$(this).attr("id")},c)});if(""!=location.hash){var g=location.hash.split("&")[0],b=location.hash.split("&")[1],
e=location.hash.split("&")[2];if(b){if("instagram"==b.split("=")[1])_instaCrossLogin(g.split("=")[1]);else if("facebook"==b.split("=")[1])_fbCrossLogin(g.split("=")[1]);else{$.removeCookie("source");$.removeCookie("access_token");parent.location.hash="";return}parent.location.hash="";$.cookie("access_token",g.split("=")[1]);e&&"b"==e.split("=")[0]&&($("body").append('<div class="modalOverlay"></div>'),"succeed"==e.split("=")[1]?new Messi($.i18n.prop("Msg_20"),{title:$.i18n.prop("Msg_21"),buttons:[{id:0,
label:"Close",val:"C"}],callback:function(){$(".modalOverlay").remove();simpleCart.empty()}}):new Messi($.i18n.prop("Msg_22"),{title:$.i18n.prop("Msg_23"),buttons:[{id:0,label:"Close",val:"C"}],callback:function(){$(".modalOverlay").remove()}}))}else $.removeCookie("source"),$.removeCookie("access_token")}else $.cookie("access_token")&&$.cookie("source")&&("instagram"==$.cookie("source")?_instaCrossLogin($.cookie("access_token")):"facebook"==$.cookie("source")?_fbCrossLogin($.cookie("access_token")):
($.removeCookie("source"),$.removeCookie("access_token"),parent.location.hash=""))};_logout=function(){$("body").append('<div class="modalOverlay"></div>');$.removeCookie("source");$.removeCookie("access_token");$.removeCookie("user_id");$.removeCookie("phone_name");$.removeCookie("layout_name");$.removeCookie("phone_color");_fblogout(_instalogout)};var h;_instalogin=function(a){$(".recent").find(".draggable").each(function(){$(this).remove();$(this)[0]=null});h="";var b="https://instagram.com/oauth/authorize/?client_id="+
INSTAGRAM_CLIENT_ID+"&amp;redirect_uri="+HOST_URL+INSTAREDIRECT_URL+"&amp;response_type=token";if(!0==a.data.isPopup)return window.open(b,"hotprintCloud","height=400,width=600,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=yes"),!1;window.location=b};_instalogout=function(){$.ajax({url:"https://instagram.com/accounts/logout/",dataType:"jsonp",error:function(){location.reload()},success:function(){location.reload()}})};_instaCrossLogin=function(a){access_token=
a;source_type="instagram";null==$.cookie("source")&&$.cookie("source",source_type);$("body").addClass("logged-in");$(".tip").fadeOut(1E3);_getUserData();f()};_getUserData=function(){$.ajax({dataType:"jsonp",url:"https://api.instagram.com/v1/users/self",data:{access_token:access_token}}).success(_displayUserData)};_displayUserData=function(a){"200"!=a.meta.code?($.removeCookie("source"),$.removeCookie("access_token"),$("body").removeClass("logged-in"),$("body").addClass("not-logged-in")):(user_id=
a.data.id,"instagram"==$.cookie("source")&&($.cookie("user_id",user_id),$.cookie("user_type","instagram"),$(".open").html("").fadeIn(300).append(a.data.username)),h="",_getUserRecent())};_getUserRecent=function(a){0<$("#fbalbums").length&&$("#fbalbums").attr("disabled","disabled");$.ajax({dataType:"jsonp",url:null!=a?a:"https://api.instagram.com/v1/users/"+user_id+"/media/recent",data:{access_token:access_token,count:100}}).success(_displayUserRecent)};_displayUserRecent=function(a){for(var b=0;b<
a.data.length;b++)h+="<li><a><img src='loader.gif' delay_src=\""+a.data[b].images.standard_resolution.url+'" delay_src_h="'+a.data[b].images.standard_resolution.url+"\" id='recent_instaimg"+b+"' class='draggable draggable-h draggable-w gallery-pool' /></a></li>";"undefined"!=typeof a.pagination.next_url?_getUserRecent(a.pagination.next_url):gallery_bar_setting(h)};var e;_fblogin=function(a){$(".recent").find(".draggable").each(function(){$(this).remove();$(this)[0]=null});e="";var b="http://www.facebook.com/dialog/oauth/?client_id="+
FACEBOOK_CLIENT_ID+"&redirect_uri="+HOST_URL+FBREDIRECT_URL+"&response_type=token&scope=user_photos";if(!0==a.data.isPopup)return window.open(b,"hotprintCloud","height=600,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=yes"),!1;window.location=b};_fbCrossLogin=function(a){$("#facebook-source").html("");access_token=a;source_type="facebook";null==$.cookie("source")&&$.cookie("source",source_type);$("body").addClass("logged-in");$(".tip").fadeOut(1E3);
f();FB.api("/me?access_token="+access_token+"&fields=albums,id,name",function(a){!a||a.error?($.removeCookie("source"),$.removeCookie("access_token"),$("body").removeClass("logged-in"),$("body").addClass("not-logged-in")):(user_id=a.id,"facebook"==$.cookie("source")&&($.cookie("user_id",user_id),$.cookie("user_type","facebook"),$(".open").html("").fadeIn(300).append(a.name)),$select=$("<select></select>"),$select.attr("id","fbalbums"),$.each(a.albums.data,function(a,b){$select.append($("<option></option>").attr("value",
b.id).text(b.name))}),$("#facebook-source").append($select),$select.change(function(){$(".recent").find(".draggable").each(function(){$(this).remove();$(this)[0]=null});e="";_facebookPhotoAlbum()}),e="",_facebookPhotoAlbum())})};_fblogout=function(a){FB.getLoginStatus(function(b){"connected"===b.status?FB.logout(function(){"undefined"!=typeof a&&a()}):"undefined"!=typeof a&&a()})};_facebookPhotoAlbum=function(a){var b=$.extend({facebookAlbumId:$("#fbalbums :selected").val(),photoLimit:"500",randomOrder:"false"}),
c=b.facebookAlbumId,d=b.photoLimit;$.getJSON(null!=a?a+"&callback=?":"https://graph.facebook.com/"+c+"/photos?access_token="+access_token+"&limit=0&callback=?",function(a){0<$("#fbalbums").length&&$("#fbalbums").attr("disabled",!1);var b=d;a.data.length<b&&(b=a.data.length);for(i=0;i<b;i++){var h=a.data[i];e=h.width>h.height?e+("<li><a><img src='loader.gif' delay_src=\""+h.source+'" delay_src_h="'+h.images[0].source+"\" id='recent_fbimg"+c+i+"' class='draggable draggable-h gallery-pool'/></a></li>"):
e+("<li><a><img src='loader.gif' delay_src=\""+h.source+'" delay_src_h="'+h.images[0].source+"\" id='recent_fbimg"+c+i+"' class='draggable draggable-w gallery-pool'/></a></li>")}"undefined"!=typeof a.paging?"undefined"!=typeof a.paging.next?_facebookPhotoAlbum(a.paging.next):gallery_bar_setting(e):gallery_bar_setting(e)})}})(jQuery);
...
label:"Yes",val:"Y"},{id:1,label:"No",val:"N"}],callback:function(c){$(".modalOverlay").remove();$(".layout_corner").animate({opacity:CORNER_OPT});"Y"===c&&($(".layout_corner").length&&$(".layout_corner").children(".draggable").each(function(){$(this).remove();$(this)[0]=null}),$(".canvas_appended").length&&$(".canvas_appended").each(function(){$(this).remove();$(this)[0]=null}),d(a))}})):d(a)}
...
val:"N"}],callback:function(c){$(".modalOverlay").remove();$(".layout_corner").animate({opacity:CORNER_OPT});"Y"===c&&($(".layout_corner").length&&$(".layout_corner").children(".draggable").each(function(){$(this).remove();$(this)[0]=null}),$(".canvas_appended").length&&$(".canvas_appended").each(function(){$(this).remove();$(this)[0]=null}),d(a))}})):d(a)}
