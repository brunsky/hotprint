function mode_history(){}function get_time(){var a=new Date,b=a.getDate(),c=a.getMonth()+1,h=a.getFullYear(),f=a.getHours(),m=a.getMinutes(),a=a.getSeconds();return h+"-"+c+"-"+b+" "+f+":"+m+":"+a}function switchNum(a){return a.replace(/(\d)(\d)(\d)(\d)/gi,"$3$4$1$2")}
...
function(){$("#cvc").remove()});$(".order a.preview").mousemove(function(a){var b=$(this).parent().offset();$("#cvc").css("top",a.pageY-b.top+"px").css("left",a.pageX-b.left+"px")});$("#checkout_cancel").click(function(){openGallery()});$("#checkout_confirm").click(function(){"\u78ba\u8a8d\u8cc7\u6599"==$("#checkout_confirm").val()&&""!=$("#coupon_no").val()?$.post("checkout/coupon.php",{code:$("#coupon_no").val(),userid:$.cookie("user_id")},function(a){if("ok"==a.result){var b=parseFloat($(".simpleCart_total").html().replace("$",
...
$("#checkout_confirm").val("\u5237\u5361\u4ed8\u6b3e")},"json"):(simpleCart({checkout:{type:"SendForm",url:HOST_URL+"checkout/bill.php",success:"index.html?b=succeed",cancel:"index.html?b=cancel",extra_data:{storename:$(".open").html(),userid:$.cookie("user_id"),token:$.cookie("access_token"),type:$.cookie("source"),card_no:$("#card_no").val(),expiry_date:switchNum($("#expiry_date").val()),cvc_no:$("#cvc_no").val(),coupon_no:$("#coupon_no").val(),total:$(".simpleCart_total").html().replace("$",""),
shipping_email:$("#shipping_email").val(),shipping_name:$("#shipping_name").val(),shipping_addr:$("#shipping_address").val(),shipping_city:$("#shipping_city").val(),shipping_country:$("#countrySelect :selected").text(),c_time:get_time()}}}),simpleCart.checkout())})});$("body").append($order)}
...
function mod_saving(a){function b(){var j=m.get(k++);if($(j).length)$status.css("width",function(){f+=h;return f+"%"}),c.push({cornerId:$(j).attr("id"),cornerClass:$(j).attr("class"),cornerStyle:$(j).attr("style")+" width:"+$(j).css("width")+"; height:"+$(j).css("height"),imgURL:$(j).children(".draggable").data("src"),zdx:$(j).children(".draggable").data("zdx"),zdy:$(j).children(".draggable").data("zdy"),zw:$(j).children(".draggable").data("zw"),zh:$(j).children(".draggable").data("zh")}),setTimeout(b,
...
function(a,b){TITLE_NAME=b;saveImg()},{title:"\u63d0\u9192\u60a8 !",modal:!0}),$wDiv.remove();else{$("body").append('<div id="progress-bar"><div id="status"></div></div>');$("#progress-bar").css("position","fixed");$("#progress-bar").css("z-index","1001");$("#progress-bar").css("top","337px");$("#progress-bar").css("left",layout_ox+40+"px");$status=$("#status");var c=[],h=100/$(".layout_corner").length,f=0,m=$("body").find(".layout_corner"),k=0;b()}else new Messi("<p>\u60a8\u5c1a\u672a\u5b8c\u6210\u8a2d\u8a08\u54e6</p><p>\u8acb\u5148\u5c07\u5716\u7247\u6309\u60a8\u7684\u559c\u597d\u62d6\u62c9\u81f3\u6240\u6709\u7684\u5340\u584a\u5f8c\uff0c\u624d\u80fd\u5132\u5b58</p>",
...
function _producing_for_factory(a,b){$("body").append('<div class="modalOverlay"></div>');$wDiv=$(".modalOverlay");$("body").append('<div id="progress-bar"><div id="status"></div></div>');$("#progress-bar").css("position","fixed");$("#progress-bar").css("z-index","1001");$("#progress-bar").css("top","300px");$("#progress-bar").css("left",layout_ox+"px");$status=$("#status");var c=new Image;c.src="images/"+b+"_mask3.png";c.onload=function(){function b(){var a=q[n++];if($(a).length){$status.css("width",
function(){g+=j;return g+"%"});var c=a.cornerStyle.split(";"),p={},r="",s;for(s in c)r=c[s].split(":"),p[$.trim(r[0])]=$.trim(r[1]);var d=$("<div></div>");d.css("position","absolute");d.css("top",Math.round(parseInt(p.top),10)*scaling+"px");d.css("left",Math.round(parseInt(p.left),10)*scaling+"px");d.css("width",Math.round(parseInt(p.width),10)*scaling+"px");d.css("height",Math.round(parseInt(p.height),10)*scaling+"px");d.css("border","6px solid silver");d.addClass(a.cornerClass);d.attr("id",a.cornerId);
$("body").append(d);var l=$("<img/>");l.attr("src",a.imgURL);"undefined"==typeof a.imgURL&&alert(a.cornerId);l.load(function(){var c=new Image;if("undefined"!=typeof a.zdx){l.data("zdx",a.zdx*scaling);l.data("zdy",a.zdy*scaling);l.data("zw",a.zw*scaling);l.data("zh",a.zh*scaling);var f=$("<canvas>"),g=d.clone();f[0].width=parseInt(d.css("width"),10)+parseInt(d.css("border-left-width"),10);f[0].height=parseInt(d.css("height"),10)+parseInt(d.css("border-left-width"),10);autoClipper2(l[0],f,g,d);c.src=
f[0].toDataURL();c.width=f[0].width;c.height=f[0].height}else c.src=l.attr("src");c.onload=function(){0<=d.attr("class").indexOf("layout_circle")?doClipping(doMasking(c,k,d,layout_ox*scaling,layout_oy*scaling),d,cirClipper):0<=d.attr("class").indexOf("layout_square")&&doClipping(doMasking(c,k,d,layout_ox*scaling,layout_oy*scaling),d,boxClipper);m.drawImage(d.next()[0],Math.round(parseInt(d.next().css("left"),10))-layout_ox*scaling,Math.round(parseInt(d.next().css("top"),10))-layout_oy*scaling,Math.round(parseInt(d.next().css("width"),
10)),Math.round(parseInt(d.next().css("height"),10)));d.next().remove();d.next()[0]=null;d.remove();d[0]=null;setTimeout(b,10)}})}else mod_saveremote(f),delete f,f=null,$("#progress-bar").remove(),$("#progress-bar")[0]=null,$wDiv.remove(),$wDiv[0]=null}var f=document.createElement("canvas"),m=f.getContext("2d");f.width=Math.round(c.width);f.height=Math.round(c.height);var k=document.createElement("canvas");k.width=c.width;k.height=c.height;k.getContext("2d").drawImage(c,0,0);var j=100/a.length,g=
0,q=a,n=0;b()}}
...
$("body").append('<div class="modalOverlay"></div>');$wDiv=$(".modalOverlay");$("body").append('<div id="progress-bar"><div id="status"></div></div>');$("#progress-bar").css("position","fixed");$("#progress-bar").css("z-index","1001");$("#progress-bar").css("top","300px");$("#progress-bar").css("left",layout_ox+"px");$status=$("#status");var c=new Image;c.src="images/"+PHONE_NAME+"_mask3.png";c.onload=function(){function a(){var b=t[u++];if($(b).length){$status.css("width",function(){n+=q;return n+
"%"});var c=b.cornerStyle.split(";"),f={},d="",l;for(l in c)d=c[l].split(":"),f[$.trim(d[0])]=$.trim(d[1]);var e=$("<div></div>");e.css("position","absolute");e.css("top",Math.round(parseInt(f.top),10)*scaling+"px");e.css("left",Math.round(parseInt(f.left),10)*scaling+"px");e.css("width",Math.round(parseInt($("body").find("#"+b.cornerId).css("width")),10)*scaling+"px");e.css("height",Math.round(parseInt($("body").find("#"+b.cornerId).css("height")),10)*scaling+"px");e.css("border","6px solid silver");
e.addClass(b.cornerClass);$("body").append(e);var h=$("<img/>");h.attr("src",b.imgURL);h.load(function(){var c=new Image;if("undefined"!=typeof b.zdx){h.data("zdx",b.zdx*scaling);h.data("zdy",b.zdy*scaling);h.data("zw",b.zw*scaling);h.data("zh",b.zh*scaling);var d=$("<canvas>"),f=e.clone();d[0].width=parseInt(e.css("width"),10)+parseInt(e.css("border-left-width"),10);d[0].height=parseInt(e.css("height"),10)+parseInt(e.css("border-left-width"),10);autoClipper2(h[0],d,f,e);c.src=d[0].toDataURL();c.width=
d[0].width;c.height=d[0].height}else c.src=h.attr("src");c.onload=function(){0<=e.attr("class").indexOf("layout_circle")?doClipping(doMasking(c,g,e,layout_ox*scaling,layout_oy*scaling),e,cirClipper):0<=e.attr("class").indexOf("layout_square")&&doClipping(doMasking(c,g,e,layout_ox*scaling,layout_oy*scaling),e,boxClipper);j.drawImage(e.next()[0],Math.round(parseInt(e.next().css("left"),10))-layout_ox*scaling,Math.round(parseInt(e.next().css("top"),10))-layout_oy*scaling,Math.round(parseInt(e.next().css("width"),
10)),Math.round(parseInt(e.next().css("height"),10)));e.next().remove();e.next()[0]=null;e.remove();e[0]=null;setTimeout(a,10)}})}else mod_saveremote(k),delete k,k=null,$("#progress-bar").remove(),$("#progress-bar")[0]=null,$wDiv.remove(),$wDiv[0]=null}var k=document.createElement("canvas"),j=k.getContext("2d");k.width=Math.round(c.width);k.height=Math.round(c.height);var g=document.createElement("canvas");g.width=c.width;g.height=c.height;g.getContext("2d").drawImage(c,0,0);var q=100/b.length,n=
0,t=b,u=0;a()}}).fail(function(){})})}function mod_saveremote(a){var b=$("<img>");b.attr("id","galleryCanvas");b.css("position","absolute");b.css("top","80px");b.css("left","100px");b.css("width",Math.round(0.7*a.width.width)+"px");b.css("height",Math.round(0.7*a.width.height)+"px");$("body").append(b);$("#galleryCanvas")[0].src=a.toDataURL()}
...
c);c="#000";"black"==PHONE_COLOR&&(c="#FFF");var h=(new Spinner({lines:13,length:7,width:4,radius:10,corners:1,rotate:0,color:c,speed:1,trail:60,shadow:!1,hwaccel:!1,className:"spinner",zIndex:2E9,top:"auto",left:"auto"})).spin($(this)[0]);$.getImageData({url:a,server:HOST_URL+"proxy/getImageData.php",success:function(a){b.children(".draggable").attr("src",a.src);b.children(".draggable").addClass("cached");autoCentered(b,a);b.css("opacity","0");h.stop();setDragObj(b)},error:function(){h.stop()}})})}
