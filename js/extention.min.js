...
function get_time(){var a=new Date,b=a.getDate(),c=a.getMonth()+1,e=a.getFullYear(),f=a.getHours(),l=a.getMinutes(),a=a.getSeconds();return e+"-"+c+"-"+b+" "+f+":"+l+":"+a}function switchNum(a){return a.replace(/(\d)(\d)(\d)(\d)/gi,"$3$4$1$2")}function checkValid(){var a=!0;$(".order :input").each(function(){""==$(this).val()&&"coupon_no"!=$(this).attr("id")&&($(this).after("<span class='fillcheck'><font color='red'>"+$.i18n.prop("Msg_51")+"<font></span>"),a=!1)});return a}
...
function _producing_for_factory(a,b){$("body").append('<div class="modalOverlay"></div>');$wDiv=$(".modalOverlay");$("body").append('<div id="progress-bar"><div id="status"></div></div>');$("#progress-bar").css("position","fixed");$("#progress-bar").css("z-index","1001");$("#progress-bar").css("top","300px");$("#progress-bar").css("left",layout_ox+"px");$status=$("#status");var c=new Image;c.src="images/"+b+"_mask3.png";c.onload=function(){function b(){var a=r[p++];if($(a).length){var c=a.imgURL.split("hotprintcloud.com/proxy/tmp/")[1].replace(/\+/g,
"/");console.log("ready to get : "+c+" from:"+HOST_URL+"proxy/getImageData.php");$.ajax({type:"GET",url:HOST_URL+"proxy/getImageData.php",data:{url:c},async:!1});console.log("get complete");$status.css("width",function(){j+=k;return j+"%"});var c=a.cornerStyle.split(";"),q={},s="",t;for(t in c)s=c[t].split(":"),q[$.trim(s[0])]=$.trim(s[1]);var d=$("<div></div>");d.css("position","absolute");d.css("top",Math.round(parseInt(q.top),10)*scaling+"px");d.css("left",Math.round(parseInt(q.left),10)*scaling+
"px");d.css("width",Math.round(parseInt(q.width),10)*scaling+"px");d.css("height",Math.round(parseInt(q.height),10)*scaling+"px");d.css("border","6px solid silver");d.addClass(a.cornerClass);d.attr("id",a.cornerId);$("body").append(d);var m=$("<img/>");m.attr("src",a.imgURL);"undefined"==typeof a.imgURL&&alert(a.cornerId);console.log("loading..."+a.imgURL);m.load(function(){var c=new Image;if("undefined"!=typeof a.zdx){m.data("zdx",a.zdx*scaling);m.data("zdy",a.zdy*scaling);m.data("zw",a.zw*scaling);
m.data("zh",a.zh*scaling);var f=$("<canvas>"),j=d.clone();f[0].width=parseInt(d.css("width"),10)+parseInt(d.css("border-left-width"),10);f[0].height=parseInt(d.css("height"),10)+parseInt(d.css("border-left-width"),10);autoClipper2(m[0],f,j,d);c.src=f[0].toDataURL();c.width=f[0].width;c.height=f[0].height}else c.src=m.attr("src");c.onload=function(){0<=d.attr("class").indexOf("layout_circle")?doClipping(doMasking(c,h,d,layout_ox*scaling,layout_oy*scaling),d,cirClipper):0<=d.attr("class").indexOf("layout_square")&&
doClipping(doMasking(c,h,d,layout_ox*scaling,layout_oy*scaling),d,boxClipper);l.drawImage(d.next()[0],Math.round(parseInt(d.next().css("left"),10))-layout_ox*scaling,Math.round(parseInt(d.next().css("top"),10))-layout_oy*scaling,Math.round(parseInt(d.next().css("width"),10)),Math.round(parseInt(d.next().css("height"),10)));d.next().remove();d.next()[0]=null;d.remove();d[0]=null;setTimeout(b,10)}})}else mod_saveremote(f),delete f,f=null,$("#progress-bar").remove(),$("#progress-bar")[0]=null,$wDiv.remove(),
$wDiv[0]=null}var f=document.createElement("canvas"),l=f.getContext("2d");f.width=Math.round(c.width);f.height=Math.round(c.height);var h=document.createElement("canvas");h.width=c.width;h.height=c.height;h.getContext("2d").drawImage(c,0,0);var k=100/a.length,j=0,r=a,p=0;b()}}
function _producing_for_factory_from_layout(a){var b=document.createElement("Canvas");b.setAttribute("id","mCanvas");b.getContext("2d");$(".mainmenu").before(b);$("link[type='text/css']#layout_css").remove();b=jQuery("<link>");b.attr({rel:"stylesheet",type:"text/css",href:"css/"+a.phone_type+"_"+a.layout_no+".css?v="+(new Date).getTime(),id:"layout_css"});$("head").append(b);styleOnload(b[0],function(){var b="js/"+a.phone_type+"_"+a.layout_no+".js?v="+(new Date).getTime();$.getScript(b).done(function(){var b=
$.parseJSON(a.saveimag);$("body").append('<div class="modalOverlay"></div>');$wDiv=$(".modalOverlay");$("body").append('<div id="progress-bar"><div id="status"></div></div>');$("#progress-bar").css("position","fixed");$("#progress-bar").css("z-index","1001");$("#progress-bar").css("top","300px");$("#progress-bar").css("left",layout_ox+"px");$status=$("#status");var c=new Image;c.src="images/"+PHONE_NAME+"_mask3.png";c.onload=function(){function a(){var b=u[v++];if($(b).length){$status.css("width",
function(){p+=r;return p+"%"});var c=b.cornerStyle.split(";"),e={},d="",f;for(f in c)d=c[f].split(":"),e[$.trim(d[0])]=$.trim(d[1]);var g=$("<div></div>");g.css("position","absolute");g.css("top",Math.round(parseInt(e.top),10)*scaling+"px");g.css("left",Math.round(parseInt(e.left),10)*scaling+"px");g.css("width",Math.round(parseInt($("body").find("#"+b.cornerId).css("width")),10)*scaling+"px");g.css("height",Math.round(parseInt($("body").find("#"+b.cornerId).css("height")),10)*scaling+"px");g.css("border",
"6px solid silver");g.addClass(b.cornerClass);$("body").append(g);var n=$("<img/>");n.attr("src",b.imgURL);n.load(function(){var c=new Image;if("undefined"!=typeof b.zdx){n.data("zdx",b.zdx*scaling);n.data("zdy",b.zdy*scaling);n.data("zw",b.zw*scaling);n.data("zh",b.zh*scaling);var d=$("<canvas>"),e=g.clone();d[0].width=parseInt(g.css("width"),10)+parseInt(g.css("border-left-width"),10);d[0].height=parseInt(g.css("height"),10)+parseInt(g.css("border-left-width"),10);autoClipper2(n[0],d,e,g);c.src=
d[0].toDataURL();c.width=d[0].width;c.height=d[0].height}else c.src=n.attr("src");c.onload=function(){0<=g.attr("class").indexOf("layout_circle")?doClipping(doMasking(c,j,g,layout_ox*scaling,layout_oy*scaling),g,cirClipper):0<=g.attr("class").indexOf("layout_square")&&doClipping(doMasking(c,j,g,layout_ox*scaling,layout_oy*scaling),g,boxClipper);k.drawImage(g.next()[0],Math.round(parseInt(g.next().css("left"),10))-layout_ox*scaling,Math.round(parseInt(g.next().css("top"),10))-layout_oy*scaling,Math.round(parseInt(g.next().css("width"),
