...
function _producing_for_factory(a,b){$("body").append('<div class="modalOverlay"></div>');$wDiv=$(".modalOverlay");$("body").append('<div id="progress-bar"><div id="status"></div></div>');$("#progress-bar").css("position","fixed");$("#progress-bar").css("z-index","1001");$("#progress-bar").css("top","300px");$("#progress-bar").css("left",layout_ox+"px");$status=$("#status");var c=new Image;c.src="images/"+b+"_mask3.png";c.onload=function(){function b(){var a=r[p++];if($(a).length){$.ajax({type:"HEAD",
url:a.imgURL,async:!1,success:function(){return!0},error:function(){return!1}});$status.css("width",function(){j+=k;return j+"%"});var c=a.cornerStyle.split(";"),q={},s="",t;for(t in c)s=c[t].split(":"),q[$.trim(s[0])]=$.trim(s[1]);var d=$("<div></div>");d.css("position","absolute");d.css("top",Math.round(parseInt(q.top),10)*scaling+"px");d.css("left",Math.round(parseInt(q.left),10)*scaling+"px");d.css("width",Math.round(parseInt(q.width),10)*scaling+"px");d.css("height",Math.round(parseInt(q.height),
10)*scaling+"px");d.css("border","6px solid silver");d.addClass(a.cornerClass);d.attr("id",a.cornerId);$("body").append(d);var m=$("<img/>");m.attr("src",a.imgURL);"undefined"==typeof a.imgURL&&alert(a.cornerId);console.log("loading..."+a.imgURL);m.load(function(){var c=new Image;if("undefined"!=typeof a.zdx){m.data("zdx",a.zdx*scaling);m.data("zdy",a.zdy*scaling);m.data("zw",a.zw*scaling);m.data("zh",a.zh*scaling);var f=$("<canvas>"),j=d.clone();f[0].width=parseInt(d.css("width"),10)+parseInt(d.css("border-left-width"),
10);f[0].height=parseInt(d.css("height"),10)+parseInt(d.css("border-left-width"),10);autoClipper2(m[0],f,j,d);c.src=f[0].toDataURL();c.width=f[0].width;c.height=f[0].height}else c.src=m.attr("src");c.onload=function(){0<=d.attr("class").indexOf("layout_circle")?doClipping(doMasking(c,h,d,layout_ox*scaling,layout_oy*scaling),d,cirClipper):0<=d.attr("class").indexOf("layout_square")&&doClipping(doMasking(c,h,d,layout_ox*scaling,layout_oy*scaling),d,boxClipper);l.drawImage(d.next()[0],Math.round(parseInt(d.next().css("left"),
10))-layout_ox*scaling,Math.round(parseInt(d.next().css("top"),10))-layout_oy*scaling,Math.round(parseInt(d.next().css("width"),10)),Math.round(parseInt(d.next().css("height"),10)));d.next().remove();d.next()[0]=null;d.remove();d[0]=null;setTimeout(b,10)}})}else mod_saveremote(f),delete f,f=null,$("#progress-bar").remove(),$("#progress-bar")[0]=null,$wDiv.remove(),$wDiv[0]=null}var f=document.createElement("canvas"),l=f.getContext("2d");f.width=Math.round(c.width);f.height=Math.round(c.height);var h=
document.createElement("canvas");h.width=c.width;h.height=c.height;h.getContext("2d").drawImage(c,0,0);var k=100/a.length,j=0,r=a,p=0;b()}}
