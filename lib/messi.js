  ...
  if(_this.options.buttons.length > 0) {
    ...
    for (var i = 0; i < _this.options.buttons.length; i++) {
      ...
      btn.bind('click', function() {
        ...
        var content = $('.messi-content input').val();
        if ($('.messi-content input').length > 0 && $.trim(content) === '')
        	return;
        var after = (_this.options.callback != null) ? function() { _this.options.callback(value, content); } : null;
        _this.hide(after);
      });
