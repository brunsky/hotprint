...
}}else{h=constraintH;w=width*constraintH/height;if(w>constraintW){w=constraintW;h=height*constraintW/width;}}}}else{if(mode==this.AUTO_FILL){if(constraintH>constraintW){h=constraintH;w=width*constraintH/height;if(w<constraintW){w=constraintW;h=height*constraintW/width;}}else{w=constraintW;h=height*constraintW/width;if(h<constraintH){h=constraintH;w=width*constraintH/height;}}}}}}}var tx=(constraintW-w)/2;var ty=(constraintH-h)/2;switch(alignment){case DC.Alignment.LEFT:tx=0;break;case DC.Alignment.RIGHT:tx=constraintW-w;break;case DC.Alignment.TOP:ty=0;break;case DC.Alignment.BOTTOM:ty=(constraintH-h);break;}return{width:w,height:h,tx:tx,ty:ty};}};DC.HCI={DEFAULT_EVENT:function(e){e.preventDefault();},disableDrag:function(){jQuery("body").bind("dragstart",this.DEFAULT_EVENT);},enableDrag:function(){jQuery("body").unbind("dragstart",this.DEFAULT_EVENT);},disableTouch:function(){jQuery(document).bind("touchmove",this.DEFAULT_EVENT);jQuery(document).bind("MozTouchMove",this.DEFAULT_EVENT);},enableTouch:function(){jQuery(document).unbind("touchmove");jQuery(document).unbind("moztouchmove");}};}(window));(function(window){DC.Event={type:"event",target:null,init:function(type){this.type=type;},clone:function(){return new Event(this.type);},toString:function(){return"[Event (type="+this.type+")]";}};DC.Event=Class.extend(DC.Event);DC.EventDispatcher={_listener:null,init:function(options){this._listener=[];this._super(options);},IdispatchEvent:function(e){if(e&&this._listener[e.type]){for(var i in this._listener[e.type]){this._listener[e.type][i].call(e.target,e);}}},IaddEventListener:function(type,f){this._listener[type]=this._listener[type]||[];this._listener[type].push(f);},IremoveEventListener:function(type,f){if(f&&this._listener[type]){for(var i in this._listener[type]){if(this._listener[type][i]==f){this._listener[type].splice(i,0);}}}else{this._listener[type]=null;delete this._listener[type];}}};DC.EventDispatcher=Interface.extend(DC.EventDispatcher);DC.ErrorEvent={error:null,init:function(type,error){this._super(type);this.error=error;}};DC.ErrorEvent=DC.Event.extend(DC.ErrorEvent);jQuery.extend(DC.ErrorEvent,{ERROR:"error"});}(window));(function(window){var XMLParser={parse:null};DC.XMLParser=Class.extend(XMLParser);var DCXMLParser={parse:function(xml,options){if(jQuery(xml).find("items").size()>0){var result=[];jQuery(xml).find("item").each(function(){var node=jQuery(this);var item={};item["source"]=options.imagePath+node.attr("source");item["thumbnail"]=options.imagePath+node.attr("thumbnail");item["category"]=node.attr("category");jQuery(this).children().each(function(){item[this.nodeName]=jQuery(this).text();});result.push(item);});return result;}else{return null;}}};DC.DCXMLParser=DC.XMLParser.extend(DCXMLParser);var LRXMLParser={parse:function(xml,options){if(jQuery(xml).find("mediaGroup").size()>0){var result=[];jQuery(xml).find("item").each(function(){var item={};if(DC.System.isIE){if(jQuery(this).find("description").get(0)){item.description=jQuery(this).find("description").get(0).text;}if(jQuery(this).find("title").get(0)){item.title=jQuery(this).find("title").get(0).text;}if(jQuery(this).find("category").get(0)){item.category=jQuery(this).find("category").get(0).text;}}else{if(jQuery(this).find("description").get(0)){item.description=jQuery(this).find("description").get(0).textContent;}if(jQuery(this).find("title").get(0)){item.title=jQuery(this).find("title").get(0).textContent;}if(jQuery(this).find("category").get(0)){item.category=jQuery(this).find("category").get(0).textContent;}}jQuery(this).find("rendition").each(function(){var node=jQuery(this);var size=node.attr("size");var source=node.attr("src");var w=node.attr("width");var h=node.attr("height");switch(size){case"small":item.smallSource=imagePath+source;item.smallWidth=w;item.smallHeight=h;break;case"medium":item.mediumSource=imagePath+source;item.mediumWidth=w;item.mediumHeight=h;break;case"large":item.largeSource=imagePath+source;item.largeWidth=w;item.largeHeight=h;break;default:item.thumbSource=imagePath+source;item.thumbWidth=w;item.thumbHeight=h;break;}item.thumbnail=item.thumbSource;item.source=item.largeSource;item.width=w;item.height=h;});result.push(item);});return result;}else{return null;}}};DC.LRXMLParser=DC.XMLParser.extend(LRXMLParser);var PSXMLParser={parse:function(xml,options){if(jQuery(xml).find("gallery").size()>0){var result=[];var thumbpath=imagePath+jQuery(xml).find("thumbnail").attr("base");var largepath=imagePath+jQuery(xml).find("large").attr("base");jQuery(xml).find("image").each(function(){var item={};var node=jQuery(this);item.thumbnail=thumbpath+node.attr("thumbnpath");item.source=largepath+node.attr("path");item.width=node.attr("width");item.height=node.attr("height");result.push(item);});return result;}else{return null;}}};DC.PSXMLParser=DC.XMLParser.extend(PSXMLParser);}(window));(function(window){DC.DataSourceEvent={index:-1,data:null,init:function(type,data,index){this._super(type);this.data=data;if(index!=undefined){this.index=index;}}};window.DC.DataSourceEvent=DC.Event.extend(DC.DataSourceEvent);jQuery.extend(DC.DataSourceEvent,{COMPLETE:"datasource_complete",UPDATE:"datasource_update",DATA_READY:"data_ready",DATA_ADD:"data_add",DATA_REMOVE:"data_remove"});DC.DataSource={source:null,relativepath:true,data:null,loaded:false,onError:DC.NOOP,onComplete:DC.NOOP,onUpdate:DC.NOOP,onDataReady:DC.NOOP,init:function(options){jQuery.extend(this,options);if(this.source){this.load(this.source);}},processData:function(){},handleDataLoadComplete:function(data){this.data=data;this.loaded=true;this.onComplete(new DC.DataSourceEvent(DC.DataSourceEvent.COMPLETE,data));},length:function(){return this.data.length;},getItemAt:function(index){return this.data?this.data[index]:null;},addItem:function(data){if(this.data){this.addItemAt(data,this.data.length);}},addItemAt:function(data,index){if(this.data){if(index>=this.data.length){this.data.push(data);}else{this.data.splice(index,0,data);index=this.data.length-1;}this.onDataUpdate(new DC.DataSourceEvent(DC.DataSourceEvent.DATA_ADD,data,index));}},removeItem:function(data){for(var i in this.data){if(this.data[i]==data){this.removeItemAt(i);}}},removeItemAt:function(index){if(this.data&&index>=0){if(index>=this.data.length){this.data.pop();}else{this.data.splice(index,1);}this.onDataUpdate(new DC.DataSourceEvent(DC.DataSourceEvent.DATA_REMOVE,this.data,index));}},load:function(src){this.loaded=false;if(src){if((src.constructor.toString().indexOf("Array")!=-1)){this.handleDataLoadComplete(src);}}}};window.DC.DataSource=Class.extend(DC.DataSource);}(window));(function(window){var XMLDataSource={parsers:[DC.DCXMLParser,DC.LRXMLParser,DC.PSXMLParser],parse:function(xml){var result=[];var imagePathArray;var imagePath="";if(this.relativepath==true){imagePathArray=this.source.split("/");imagePathArray.pop();if(imagePathArray.length>0){imagePath=imagePathArray.join("/")+"/";}}var options={imagePath:imagePath};for(idx in this.parsers){var parser=new this.parsers[idx];result=parser.parse(xml,options);if(result){break;}}return result;},load:function(src){this._super(src);var self=this;self.source=src;if(src!=""&&src!=undefined){if(jQuery.isArray(src)||jQuery.isPlainObject(src)){}else{if(jQuery.browser.msie){var xdr=false;if(!xdr){try{xdr=new ActiveXObject("MSXML2.XMLHTTP");}catch(e){xdr=false;}}if(!xdr){try{xdr=new ActiveXObject("Microsoft.XMLHTTP");}catch(e){xdr=false;}}if(!xdr&&window.XDomainRequest){try{xdr=new XDomainRequest();}catch(e){xdr=false;}}xdr.open("get",src);xdr.onreadystatechange=function(){this.onreadystatechange=null;if(xdr.readyState==4){var dom=new ActiveXObject("Microsoft.XMLDOM");dom.async=false;dom.loadXML(xdr.responseText);var results=self.parse(dom);self.handleDataLoadComplete(results);}};xdr.send();}else{jQuery.ajax({url:src,dataType:DC.System.isIE?"text":"xml",success:function(data){this.success=null;var xml;if(typeof data=="string"){xml=new ActiveXObject("Microsoft.XMLDOM");xml.async=false;xml.loadXML(data);}else{xml=data;}var results=self.parse(xml);self.handleDataLoadComplete(results);}});}}}}};DC.XMLDataSource=DC.DataSource.extend(XMLDataSource);}(window));(function(window){DC.Storage={id:"DEFAULT_STORAGE",version:"1.0",data:null,lastupdate:0,onUpdate:DC.NOOP,init:function(options){if(options){jQuery.extend(this,options);
...
{ pos.left+=target.parent._paddingB.left;pos.top+=target.parent._paddingB.top; 
   }
  catch(err)
   {
